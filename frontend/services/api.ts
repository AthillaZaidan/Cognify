...
function getApiBase(): string {
  ...
  // Prefer explicit env in all cases.
  if (fromEnv) return fromEnv;
  // On web, default to localhost (same machine backend).
  if (Platform.OS === 'web') return 'http://localhost:8000';
  // On native, prefer app.json extra.apiUrl (LAN IP).
  if (fromExtra) return fromExtra;
  return 'http://localhost:8000';
}

function makeApi() {
  return axios.create({ baseURL: getApiBase(), timeout: 8000 });
}
...
export const login = (body: LoginRequest) =>
  tryApi(
    () => makeApi().post<LoginResponse>('/login', body).then((r) => r.data),
    () => MOCK.login(body.email),
  );

export const getDashboard = (userId: number) =>
  tryApi(
    () => makeApi().get<DashboardResponse>(`/dashboard/${userId}`).then((r) => r.data),
    () => MOCK.dashboard(),
  );
...
export const getTrends = (userId: number, days = 7) =>
  tryApi(
    () =>
      makeApi()
        .get<TrendDay[]>(`/behavioral-trends/${userId}`, { params: { days } })
        .then((r) => r.data),
    () => MOCK.trends(),
  );
...
export const postBehavioral = (body: BehavioralLogRequest) =>
  tryApi(
    () => makeApi().post<BehavioralLogResponse>('/behavioral-logs', body).then((r) => r.data),
    () => ({ id: Date.now(), user_id: body.user_id, date: body.date }),
  );

export const getAnomalies = (userId: number) =>
  tryApi(
    () => makeApi().get<AnomalyLogEntry[]>(`/anomalies/${userId}`).then((r) => r.data),
    () => [],
  );

export const getInterventions = (userId: number) =>
  tryApi(
    () => makeApi().get<Intervention[]>(`/interventions/${userId}`).then((r) => r.data),
    () => MOCK.interventions(),
  );
...
export const completeIntervention = (id: number) =>
  tryApi(
    () =>
      makeApi()
        .post<{ id: number; completed: boolean; completed_at: string }>(`/interventions/${id}/complete`)
        .then((r) => r.data),
...
export const dismissIntervention = (id: number) =>
  tryApi(
    () =>
      makeApi()
        .post<{ id: number; dismissed: boolean }>(`/interventions/${id}/dismiss`)
        .then((r) => r.data),
    () => ({ id, dismissed: true }),
  );
...
export const getPatients = (psychId: number) =>
  tryApi(
    () => makeApi().get<PatientSummary[]>(`/psychologist/patients/${psychId}`).then((r) => r.data),
    () => MOCK.patients(),
  );

export const getAlerts = (psychId: number) =>
  tryApi(
    () => makeApi().get<Alert[]>(`/alerts/${psychId}`).then((r) => r.data),
    () => MOCK.alerts(),
  );
...
export const acknowledgeAlert = (id: number, notes?: string | null) =>
  tryApi(
    () =>
      makeApi()
        .post<{ id: number; acknowledged: boolean; acknowledged_at: string }>(`/alerts/${id}/acknowledge`, {
          notes: notes ?? null,
        })
        .then((r) => r.data),
...
export const analyze = (body: AnalyzeRequest) =>
  tryApi(
    () => makeApi().post<AnalyzeResponse>('/analyze', body).then((r) => r.data),
    () => ({
      risk_level: 'stable',
      alert_severity: null,
      wcs: 0.3,
